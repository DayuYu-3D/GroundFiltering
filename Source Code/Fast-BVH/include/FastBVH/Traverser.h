...
class Traverser final {
 ...
 public:
  ...
  //! \tparam Flags The flags which configure traversal.
  //! See @ref TraverserFlags. By default, Nearest flags are active.
  void setTraverserFlags(TraverserFlags Flags)
  {
      this->tFlags = Flags;
  }
  ...
  void traverse(const Ray<Float>& ray,std::vector<Intersection<Float, Primitive>>& vIntersection) const;
};
...
template <typename Float, typename Primitive, typename Intersector>
void Traverser<Float, Primitive, Intersector>::
traverse(const Ray<Float>& ray,std::vector<Intersection<Float, Primitive>>& vIntersection) const
{
    ...
    if (node.isLeaf()) {
      for (uint32_t o = 0; o < node.primitive_count; ++o) {
        ...
        if (current) {
          ...
          if (tFlags == TraverserFlags::OnlyTestOcclusion) {
            vIntersection.emplace_back(current);
            return ;
          }
          else if (tFlags == TraverserFlags::NearestOcclusion)
          {
              intersection = closest(intersection, current);
              vIntersection.clear();
              vIntersection.emplace_back(intersection);
          }
          else if (tFlags == TraverserFlags::AllOcclusion) {
              vIntersection.emplace_back(current);
          }
        }
      }
