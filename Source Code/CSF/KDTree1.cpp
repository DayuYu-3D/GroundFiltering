...
KDNodePtr KDTree::findTriangle_(
    const std::vector<std::vector<Vertex> > *pvvVertex,
    const std::vector<Triangle> *pTriangle,
    const point_t &pt, const size_t &levl, const KDNodePtr &branch, const KDNodePtr &best)
{
    if (!bool(*branch)) {//是否是叶子节点
        return NewKDNodePtr();  // basically, null
    }
    ...
    double dx;
    dx = branch_pt.at(levl) - pt.at(levl);

    KDNodePtr best_l = best;

    ///判断是否在该三角形内
    size_t geomIx = pTriangle->at(branch->index)._geomIndex;
    size_t pix1 = pTriangle->at(branch->index)._vertexIndexs[0];
    double pointA[2] = {pvvVertex->at(geomIx)[pix1]._coor[0], pvvVertex->at(geomIx)[pix1]._coor[1]};
    pix1 = pTriangle->at(branch->index)._vertexIndexs[1];
    double pointB[2] = {pvvVertex->at(geomIx)[pix1]._coor[0], pvvVertex->at(geomIx)[pix1]._coor[1]};
    pix1 = pTriangle->at(branch->index)._vertexIndexs[2];
    double pointC[2] = {pvvVertex->at(geomIx)[pix1]._coor[0], pvvVertex->at(geomIx)[pix1]._coor[1]};
    double PA[2] = {pointA[0] - pt[0], pointA[1] - pt[1]};
    double PB[2] = {pointB[0] - pt[0], pointB[1] - pt[1]};
    double PC[2] = {pointC[0] - pt[0], pointC[1] - pt[1]};
    double t1 = PA[0] * PB[1] - PA[1] * PB[0]; //叉乘
    double t2 = PB[0] * PC[1] - PB[1] * PC[0];
    double t3 = PC[0] * PA[1] - PC[1] * PA[0];
    bool isInTri = t1 * t2 >= 0 && t1 * t3 >= 0;
    if(isInTri)
    {
        best_l = branch;
        return branch;
    ...
    // keep nearest neighbor from further down the tree
    auto futher = findTriangle_(pvvVertex, pTriangle, pt, next_lv, section, best_l);
    if(!futher->x.empty())
    {
...
size_t KDTree::findTriangle_index(
    const std::vector<std::vector<Vertex> > *pvvVertex,
    const std::vector<Triangle> *pTriangle, const point_t &pt)
{
    size_t level = 0;
    auto nptr = this->findTriangle_(pvvVertex, pTriangle, pt, level, root, nullptr);
    return nptr->index;
}
